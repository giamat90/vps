...
| Pitch detection (song + take) | User-selectable: Praat (default, `praat-parselmouth`) / SRH (custom Drugman & Dutoit 2011) / pYIN / HPS / CREPE (`torchcrepe`) | — |
...
- **Praat** (`detect_pitch_praat`) — **the default**. Autocorrelation method (Boersma 1993) via `praat-parselmouth`. Added because VoceVista tracks Demucs-split vocals better than our detectors; promoted to default after lab A/B on both test tracks (fundamental held end-to-end on clean vocals, real glides preserved, meaningful per-frame confidence) plus an in-app listening test beat SRH; VoceVista's algorithm is unpublished, but its documented behavior (time-domain detector separate from the FFT, "prefer harmonic fundamental" option, pitch floor/ceiling) matches Praat's octave-cost + Viterbi-path design, and the singing-voice comparative study in `Researches/1912.12609v1` found Praat best at voicing determination. Praat defaults kept; skips `_smooth_voiced` (its path finding already smooths, same reasoning as pYIN's HMM). Parameter sweeps: `pitch_lab`'s `praat_variant()`.
...
- **Branch:** `master`
- **Current version:** `0.1.37` (as of 2026-07-11)
