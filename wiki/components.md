...
**Metronome (🥁 toggle, header row):** local component state (`metronomeEnabled` — not in the Zustand store, since nothing outside this component needs it), synced to playback rather than free-running: a `useEffect` starts/stops `src/audio/metronome.ts`'s `Metronome` singleton whenever `metronomeEnabled && isPlaying` flips, so it's silent while paused and resumes with the transport. Ticks at the effective BPM (`(detectedBpm ?? 120) * playbackRate` — falls back to 120 when the song has no detected BPM, since the rate row is always usable) and re-tunes live via `setBpm()` as tempo changes mid-playback, without resetting phase. `Metronome` uses the standard Web Audio lookahead scheduler (25 ms `setTimeout` tick, 100 ms schedule-ahead window) rather than a naive `setInterval`, so it doesn't drift under JS event-loop jitter. Every 4th click (beat 0 of an assumed 4/4 bar) is accented — higher pitch (1500 Hz vs 900 Hz) and louder gain — with no way to change the time signature, since neither project stores one. `AudioContext` is created lazily on first `start()` and suspended (not closed) on `stop()`, so re-toggling doesn't pay context-creation cost repeatedly. Not phase-locked to the song's actual downbeats (neither project stores a beat-grid/downbeat offset) — the metronome's beat 1 is just whenever playback last started.
