...
**Smoothing:** the Live/Take curve is passed through `smoothSpectrumEnvelope()` (`src/lib/spectroUtils.ts` — the same widening-window moving average `ShortTermSpectrumPanel` uses, extracted there into this shared module) before stroking; the Song reference curve is left raw/unsmoothed on purpose, since it's the precise target the singer is comparing against, not something that benefits from visual noise reduction. `storedCurvePoints()`/`liveCurvePoints()` build a `SpectrumPoint[]` first (rather than stroking per-pixel as the frame is scanned) so the smoothing pass has the whole curve to average over before `strokePoints()` draws it.
